...
try:
    ...
    print('mmcv is not installed, please install mmcv to use this code.')
...
class FreqFusion(nn.Module):
    def __init__(self,
                ...
                encoder_kernel=3,
                encoder_dilation=1,
                compressed_channels=2, # 换为比率r     compressed_channels = channels / r    
                align_corners=False,
        ...
        self.compressed_channels = int(hr_channels / compressed_channels) # 压缩为高分辨率特征通道数的一半
    ...
    def _forward(self, hr_feat, lr_feat):
        lr_feat_residual = F.interpolate(
            lr_feat,
            scale_factor=2,
        ...
        if self.use_high_pass:
            ...
            hr_feat_hf = hr_feat - carafe(hr_feat, mask_hr, self.highpass_kernel, self.up_group, 1) # identity - carafe(low_part)
            if self.hr_residual:
            ...
            if self.lr_residual:
                # print('using lr_residual')
                lr_feat = lr_feat + lr_feat_residual
            else:
                lr_feat = lr_feat
...
if __name__ == '__main__':
    ...
    hr_feat = torch.rand(8, 64, 128, 128).cuda()
    lr_feat = torch.rand(8, 128, 64, 64).cuda()
    model = FreqFusion(hr_channels=64, lr_channels=128).cuda()
    mask_lr, hr_feat_pro, lr_feat_pro = model(hr_feat=hr_feat, lr_feat=lr_feat)
    print('mask_lr', mask_lr.shape)
    print('hr_feat', hr_feat_pro.shape)# 64 512 512
    print('lr_feat', lr_feat_pro.shape)# 128 512 512       